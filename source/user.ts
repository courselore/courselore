...
export default (app: Courselore): void => {
  ...
  app.get<{}, HTML, {}, {}, IsSignedInMiddlewareLocals>(
    ...
    ...app.locals.middlewares.isSignedIn,
    (res, req) => {
      req.redirect(303, `${app.locals.options.baseURL}/administrator-panel/configuration`);
    }
  );
    ...
    (req, res) => {
      res.send(
        app.locals.layouts.administratorPanel({
          req,
          res,
          head: html`<title>Configuration · Administrator Panel · Courselore</title>`,
          body: html`
            <h2 class="heading">
              <i class="bi bi-sliders"></i>
              Administrator Panel ·
            ...
            <form
              method="PATCH"
              action="${app.locals.options.baseURL}/administrator-panel/configuration"
              novalidate
              css="${res.locals.css(css`
                display: flex;
                flex-direction: column;
    ...
    (req, res) => {
      res.send(
        app.locals.layouts.administratorPanel({
          req,
          res,
          head: html`<title>Statistics · Administrator Panel · Courselore</title>`,
          body: html`
            <h2 class="heading">
              <i class="bi bi-sliders"></i>
              Administrator Panel ·
