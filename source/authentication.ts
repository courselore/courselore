...
// import { Capacitor } from "../mobile-application/node_modules/@capacitor/core";
import {
  Courselore,
  ...
  app.patch<{}, any, { href?: string }, {}, BaseMiddlewareLocals>(
    ...
    (req, res, next) => {
      ...
      const isValidUrl = (async () => {
        const response = await fetch(`${req.body.href}/mobile-app/validation`, {
          method: "GET",
        });
        if (response.ok) {
          ...
          if (parsedText.platform === "Courselore") return true;
          return false;
        } else return false;
      })();
