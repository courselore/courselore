...
export class Database extends BetterSqlite3Database {
    constructor() {
        super(...arguments);
        _Database_statements.set(this, new Map());
    }
    execute(query) {
        let source = "";
        for (let parametersIndex = 0; parametersIndex < query.parameters.length; parametersIndex++)
            source +=
    ...
    async migrate(...migrations) {
        ...
        try {
            for (let migrationIndex = this.pragma("user_version", { simple: true }); migrationIndex < migrations.length; migrationIndex++)
                try {
                    this.execute(sql `
              BEGIN;
            `);
                    ...
                    if (foreignKeys) {
                        ...
                        if (foreignKeyViolations.length !== 0)
                            throw new Error(`Foreign key violations in migration:\n${JSON.stringify(foreignKeyViolations, undefined, 2)}`);
                    }
    ...
    getStatement(query, options = {}) {
        const source = query.sourceParts.join("?");
        let statement = __classPrivateFieldGet(this, _Database_statements, "f").get(source);
        if (statement === undefined) {
            statement = this.prepare(source);
            __classPrivateFieldGet(this, _Database_statements, "f").set(source, statement);
        }
...
_a = Database, _Database_statements = new WeakMap();
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        ...
        `);
        assert.equal(database.get(sql `SELECT * FROM "users"`).name, "Eliot Smith");
        database.close();
    }
})();
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        const database = new Database(":memory:");
        database.execute(sql `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT);`);
        assert.deepEqual(database.run(sql `INSERT INTO "users" ("name") VALUES (${"Leandro Facchinetti"})`), { changes: 1, lastInsertRowid: 1 });
        database.close();
    }
})();
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        const database = new Database(":memory:");
        database.execute(sql `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT);`);
        assert.deepEqual(database.get(sql `
            INSERT INTO "users" ("name")
            VALUES (${"Leandro Facchinetti"})
            RETURNING *
          `), { id: 1, name: "Leandro Facchinetti" });
        assert.deepEqual(database.get(sql `SELECT "id", "name" FROM "users"`), {
            id: 1,
...
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        ...
        `);
        assert.deepEqual(database.all(sql `
        ...
        assert.deepEqual(database.all(sql `SELECT "id", "name" FROM "users" WHERE "name" IN ${[]}`), []);
        assert.deepEqual(database.all(sql `SELECT "id", "name" FROM "users" WHERE "name" IN ${[
            "Leandro Facchinetti",
            "David Adler",
        ]}`), [
        ...
        assert.deepEqual(database.all(sql `SELECT "id", "name" FROM "users" WHERE "name" IN ${new Set([
            "Leandro Facchinetti",
            "David Adler",
        ])}`), [
            {
...
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        ...
        database.run(sql `INSERT INTO "users" ("name") VALUES (${"Linda Renner"})`);
        assert.deepEqual([
            ...database.iterate(sql `SELECT "id", "name" FROM "users"`),
        ], [
            {
                id: 1,
                name: "Leandro Facchinetti",
...
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        ...
        assert.deepEqual(database.all(sql `
            SELECT "id", "name" FROM "users"
          `), []);
        assert.deepEqual(database.executeTransaction(() => {
            return database.run(sql `INSERT INTO "users" ("name") VALUES (${"Leandro Facchinetti"})`);
        }), {
            changes: 1,
            lastInsertRowid: 1,
        });
...
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        ...
        assert.deepEqual(database.all(sql `
            SELECT "id", "name" FROM "users"
          `), []);
        assert.deepEqual(database.executeTransactionImmediate(() => {
            return database.run(sql `INSERT INTO "users" ("name") VALUES (${"Leandro Facchinetti"})`);
        }), {
            changes: 1,
            lastInsertRowid: 1,
...
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        ...
        assert.deepEqual(database.all(sql `
            SELECT "id", "name" FROM "users"
          `), []);
        assert.deepEqual(database.executeTransactionExclusive(() => {
            return database.run(sql `INSERT INTO "users" ("name") VALUES (${"Leandro Facchinetti"})`);
        }), {
            changes: 1,
            lastInsertRowid: 1,
...
(() => {
    if (process.env.TEST === "leafac--sqlite")
        (async () => {
            const database = new Database(":memory:");
            let counter = 0;
            await database.migrate(sql `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT);`, () => {
                counter++;
            });
            ...
            await assert.rejects(async () => {
                await database.migrate(sql `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT);`, () => {
                    ...
                    throw new Error("The previous migration should succeed, but this migration should fail");
                });
            });
            assert.deepEqual(database.all(sql `
              SELECT "id", "name" FROM "users"
            ...
            `, async (database) => {
                    database.execute(sql `INSERT INTO "users" ("name") VALUES (${"Linda Renner"})`);
...
(() => {
    if (process.env.TEST === "leafac--sqlite") {
        const database = new Database(":memory:");
        database.execute(sql `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT);`);
        assert.deepEqual(database.run(sql `INSERT INTO "users" ("name") VALUES (${"Leandro Facchinetti"})`), {
            changes: 1,
            lastInsertRowid: 1,
        });
        assert.deepEqual(database.run(sql `INSERT INTO "users" ("name") VALUES (${"Linda Renner"})`, { safeIntegers: true }), {
            changes: 1,
            lastInsertRowid: 2n,
        });
        assert.deepEqual(database.run(sql `INSERT INTO "users" ("name") VALUES (${"Louie Renner"})`), {
            changes: 1,
            lastInsertRowid: 3n,
        });
        assert.deepEqual(database.run(sql `INSERT INTO "users" ("name") VALUES (${"Cadeau Renner"})`, { safeIntegers: false }), {
            changes: 1,
            lastInsertRowid: 4,
        });
        assert.deepEqual(database.get(sql `
...
export function sql(template, ...substitutions) {
    ...
    for (let substitutionsIndex = 0; substitutionsIndex < substitutions.length; substitutionsIndex++) {
        ...
        if (templatePart.endsWith("$")) {
            ...
            if (!Array.isArray(substitution.sourceParts) ||
                substitution.sourceParts.length === 0 ||
                substitution.sourceParts.some((substitutionPart) => typeof substitutionPart !== "string") ||
                !Array.isArray(substitution.parameters) ||
                substitution.sourceParts.length !== substitution.parameters.length + 1)
                throw new Error(`Failed to interpolate raw query ‘${substitution}’ because it wasn’t created with the sql\`\` tagged template literal`);
            ...
            if (substitutionQuery.sourceParts.length === 1)
                ...
                sourceParts.push(`${templatePart}${substitutionQuery.sourceParts[0]}`, ...substitutionQuery.sourceParts.slice(1, -1));
                templateParts[substitutionsIndex + 1] = `${substitutionQuery.sourceParts[substitutionQuery.sourceParts.length - 1]}${templateParts[substitutionsIndex + 1]}`;
            ...
            if (substitution.length === 0)
                ...
                sourceParts.push(`${templatePart}(`, ...new Array(substitution.length - 1).fill(","));
...
if (process.env.TEST === "leafac--sqlite") {
    assert.deepEqual(sql `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT)`, {
        sourceParts: [
            `CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT)`,
        ],
        parameters: [],
    });
    assert.deepEqual(sql `INSERT INTO "users" ("name") VALUES (${"Leandro Facchinetti"})`, {
        sourceParts: [`INSERT INTO "users" ("name") VALUES (`, `)`],
        parameters: ["Leandro Facchinetti"],
    });
    assert.deepEqual(sql `SELECT "id", "name" FROM "users" WHERE "name" IN ${[]}`, {
    ...
    assert.deepEqual(sql `SELECT "id", "name" FROM "users" WHERE "name" IN ${new Set([])}`, {
        sourceParts: [`SELECT "id", "name" FROM "users" WHERE "name" IN ()`],
        parameters: [],
    });
    assert.deepEqual(sql `SELECT "id", "name" FROM "users" WHERE "name" IN ${new Set([
    ...
    assert.deepEqual(sql `SELECT "id", "name" FROM "users" WHERE name = ${"Leandro Facchinetti"}$${sql ` AND "age" IS NOT NULL`}`, {
        sourceParts: [
    ...
    assert.deepEqual(sql `SELECT "id", "name" FROM "users" WHERE name = ${"Leandro Facchinetti"}$${sql ` AND "age" = ${31}`}`, {
        sourceParts: [
            `SELECT "id", "name" FROM "users" WHERE name = `,
            ` AND "age" = `,
            ``,
