...
export const processedCSSCache = new Map();
...
if (process.env.TEST === "leafac--css") {
    ...
    assert.equal(pageCSS(css `
        font-family: "Public Sans";
      `), "l5tnu4");
    assert.equal(prettier.format(html `$${pageCSS.toString()}`, { parser: "html" }), prettier.format(html `
        <style key="local-css">
...
export function css(inputParts, ...interpolations) {
    ...
    for (let index = 0; index < inputParts.length; index++) {
        ...
        outputParts.push(...(Array.isArray(interpolation) ? interpolation : [interpolation]));
    }
