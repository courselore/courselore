...
const greatest = async (packageName, currentVersion, options = {}) => {
    const versions = (await (0, npm_1.viewOne)(packageName, 'versions', currentVersion, options, await npmConfigFromYarn(options)));
...
const distTag = async (packageName, currentVersion, options = {}) => {
    const revision = (await (0, npm_1.viewOne)(packageName, `dist-tags.${options.distTag}`, currentVersion, {
        ...
        retry: options.retry,
    }, await npmConfigFromYarn(options))); // known type based on dist-tags.latest
...
const newest = async (packageName, currentVersion, options = {}) => {
    const result = await (0, npm_1.viewManyMemoized)(packageName, ['time', 'versions'], currentVersion, options, 0, await npmConfigFromYarn(options));
