...
const readNpmConfig = () => {
    const npmConfigToPacoteMap = {
        cafile: async (path) => {
            ...
            if (!path)
                return;
            const cadata = await promises_1.default.readFile(path, 'utf8');
    ...
    libnpmconfig_1.default.read(null, builtinsWithUserConfig).forEach((value, key) => {
        ...
        if (typeof pacoteKey === 'string') {
            ...
            (0, assign_1.default)(config, pacoteKey(normalizedValue.toString()));
        }
        else {
            config[key.match(/^[a-z]/i) ? (0, camelCase_1.default)(key) : key] = normalizedValue;
        }
    });
    config.cache = false;
    return config;
