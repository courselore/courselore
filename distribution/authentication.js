...
import { userAvatarlessBackgroundColors, } from "./index.js";
export default (app) => {
    app.locals.helpers.Session = {
        maxAge: 180 * 24 * 60 * 60 * 1000,
        open({ req, res, userId }) {
            const session = app.locals.database.get(sql `
        ...
        get({ req, res }) {
            if (req.cookies.session === undefined)
                return undefined;
            const session = app.locals.database.get(sql `SELECT "createdAt", "user" FROM "sessions" WHERE "token" = ${req.cookies.session}`);
            ...
            else if (req.header("Live-Updates") === undefined &&
                new Date(session.createdAt).getTime() <
        ...
        close({ req, res }) {
            ...
            app.locals.database.run(sql `DELETE FROM "sessions" WHERE "token" = ${req.cookies.session}`);
        },
        closeAllAndReopen({ req, res, userId }) {
            app.locals.helpers.Session.close({ req, res });
            app.locals.database.run(sql `DELETE FROM "sessions" WHERE "user" = ${userId}`);
    ...
    (async () => {
        while (true) {
          ...
          WHERE "createdAt" < ${new Date(Date.now() - app.locals.helpers.Session.maxAge).toISOString()}
        `);
    ...
    app.locals.middlewares.isSignedIn = [
        (req, res, next) => {
            ...
            if (userId === undefined)
                return next("route");
            res.locals.user = app.locals.database.get(sql `
          SELECT "id",
                 "lastSeenOnlineAt",
                 "reference",
                 "email",
    ...
    (() => {
        ...
        app.get("/", ...app.locals.middlewares.isSignedOut, app.locals.options.host === app.locals.options.canonicalHost
            ? (req, res, next) => {
                app.locals.handlers.about(req, res, next);
            }
            : handler);
        app.get("/sign-in", ...app.locals.middlewares.isSignedOut, handler);
        app.get("/sign-in", ...app.locals.middlewares.isSignedIn, (req, res) => {
    ...
    app.post("/sign-in", ...app.locals.middlewares.isSignedOut, asyncHandler(async (req, res, next) => {
        if (typeof req.body.email !== "string" ||
            req.body.email.match(app.locals.helpers.emailRegExp) === null ||
            typeof req.body.password !== "string" ||
            req.body.password.trim() === "")
            return next("validation");
        const user = app.locals.database.get(sql `SELECT "id", "password" FROM "users" WHERE "email" = ${req.body.email}`);
        if (user === undefined ||
            ...
            return res.redirect(303, `https://${app.locals.options.host}/sign-in${qs.stringify({
                redirect: req.query.redirect,
                invitation: req.query.invitation,
            }, {
                addQueryPrefix: true,
    ...
    const PasswordReset = {
        ...
        get(nonce) {
            const passwordReset = app.locals.database.get(sql `SELECT "createdAt", "user" FROM "passwordResets" WHERE "nonce" = ${nonce}`);
            app.locals.database.run(sql `
    ...
    (async () => {
        while (true) {
          ...
          WHERE "createdAt" < ${new Date(Date.now() - PasswordReset.maxAge).toISOString()}
        `);
            await new Promise((resolve) => setTimeout(resolve, 24 * 60 * 60 * 1000));
        }
    })();
    ...
    app.post("/reset-password", (req, res, next) => {
        if (typeof req.body.email !== "string" ||
            req.body.email.match(app.locals.helpers.emailRegExp) === null)
            return next("validation");
        const user = app.locals.database.get(sql `SELECT "id", "email" FROM "users" WHERE "email" = ${req.body.email}`);
        if (user === undefined) {
            app.locals.helpers.Flash.set({
                req,
                res,
            ...
            return res.redirect(303, `https://${app.locals.options.host}/reset-password${qs.stringify({
                redirect: req.query.redirect,
                invitation: req.query.invitation,
            }, {
                addQueryPrefix: true,
        ...
        const link = `https://${app.locals.options.host}/reset-password/${PasswordReset.create(user.id)}${qs.stringify({
            redirect: req.query.redirect,
            invitation: req.query.invitation,
        }, {
            addQueryPrefix: true,
    ...
    app.get("/reset-password/:passwordResetNonce", (req, res) => {
        ...
        `,
          ...
          <form
            method="POST"
            action="https://${app.locals.options
                .host}/reset-password/${PasswordReset.create(userId)}${qs.stringify({
                redirect: req.query.redirect,
                invitation: req.query.invitation,
            }, { addQueryPrefix: true })}"
            novalidate
    ...
    app.post("/reset-password/:passwordResetNonce", asyncHandler(async (req, res, next) => {
        ...
        app.locals.database.run(sql `
          UPDATE "users"
          SET "password" = ${await argon2.hash(req.body.password, app.locals.options.argon2)}
          WHERE "id" = ${userId}
        `);
    ...
    app.get("/sign-up", ...app.locals.middlewares.isSignedOut, (req, res) => {
        res.send(app.locals.layouts.box({
            req,
            res,
            head: html `
    ...
    app.get("/sign-up", ...app.locals.middlewares.isSignedIn, (req, res) => {
        res.redirect(303, `https://${app.locals.options.host}${typeof req.query.redirect === "string" &&
            req.query.redirect.trim() !== ""
            ? req.query.redirect
            : "/"}`);
    ...
    app.locals.mailers.emailVerification = ({ req, res, userId, userEmail }) => {
        ...
        const link = `https://${app.locals.options.host}/email-verification/${emailVerification.nonce}${qs.stringify({ redirect: req.originalUrl }, { addQueryPrefix: true })}`;
        app.locals.database.run(sql `
        INSERT INTO "sendEmailJobs" (
          "createdAt",
          "startAt",
    ...
    app.post("/sign-up", ...app.locals.middlewares.isSignedOut, asyncHandler(async (req, res, next) => {
        ...
        const user = app.locals.database.get(sql `
          INSERT INTO "users" (
            "createdAt",
            "lastSeenOnlineAt",
            "reference",
    ...
    app.post("/resend-verification-email", ...app.locals.middlewares.isSignedIn, (req, res) => {
        if (res.locals.user.emailVerifiedAt !== null) {
            ...
            return res.redirect(303, `https://${app.locals.options.host}${typeof req.query.redirect === "string" &&
                req.query.redirect.trim() !== ""
                ? req.query.redirect
                : "/"}`);
        }
    ...
    app.get("/email-verification/:emailVerificationNonce", ...app.locals.middlewares.isSignedIn, (req, res) => {
        const emailVerification = app.locals.database.get(sql `
    ...
    app.delete("/sign-out", ...app.locals.middlewares.isSignedIn, (req, res) => {
        app.locals.helpers.Session.close({ req, res });
        res
