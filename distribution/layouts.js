...
export default async (app) => {
    app.locals.layouts.base = ({ req, res, head, extraHeaders = html ``, body, }) => {
        ...
        <div
          ...
          onload="${javascript `
            ...
            ? javascript `
                ...
                ? (() => {
                    let emailVerification = app.locals.database.get(sql `
                    ...
                    if (emailVerification === undefined) {
                        ...
                        emailVerification = app.locals.database.get(sql `
                                    SELECT "nonce" FROM "emailVerifications" WHERE "user" = ${res.locals.user.id}
                                  `);
                    }
                    return html `
            ...
            document.querySelector('[key="theme-color--light"]').setAttribute("content", getComputedStyle(document.documentElement).getPropertyValue("--color--${res.locals.enrollment?.accentColor}--500"));
            document.querySelector('[key="theme-color--dark"]').setAttribute("content", getComputedStyle(document.documentElement).getPropertyValue("--color--${res.locals.enrollment?.accentColor}--600"));

            ${res.locals.liveUpdatesNonce !== undefined
            ? javascript `
                    if (event?.detail?.liveUpdate !== true)
                      leafac.liveUpdates(${JSON.stringify(res.locals.liveUpdatesNonce)});
                  `
            : javascript ``}
          `}"
        >
      ...
      <html lang="en">
        <head>
          ...
          <script>
            ...
            ? javascript `
                  leafac.liveReload(${JSON.stringify(`https://${app.locals.options.host}/live-reload`)});
                `
            : javascript ``};
          </script>
    ...
    if (app.locals.options.environment === "development")
        await fs.writeFile(new URL("../static/global.css", import.meta.url), processCSS(css `
        [live-navigating] * {
        ...
        .button {
          ...
          ${["blue", "green", "rose", "amber"].map((color) => css `
              &.button--${color} {
      ...
      `));
    app.locals.layouts.box = ({ req, res, head, body }) => app.locals.layouts.base({
        req,
        res,
        head,
        body: html `
    ...
    app.locals.layouts.application = ({ req, res, head, showCourseSwitcher = true, extraHeaders = html ``, body, }) => app.locals.layouts.base({
        req,
        res,
        head,
        extraHeaders: html `
          ...
          >
            ...
            return res.locals.course !== undefined
                ? html `
                    ...
                    .host}/courses/${res.locals.course
                    .reference}"
                                      class="dropdown--menu--item button ${req.path.includes("/settings/")
                    ? "button--transparent"
                    : "button--blue"}"
                                    >
                                      <i
                                        class="bi ${req.path.includes("/settings/")
                    ? "bi-chat-text"
                    : "bi-chat-text-fill"}"
                                      ></i>
                                      Conversations
          ...
          <div>
            <button
              ...
              onload="${javascript `
                (this.tooltip ??= tippy(this)).setProps({
                  touch: false,
                  content: ${JSON.stringify(res.locals.invitations.length === 0
            ? "Add"
            : `${res.locals.invitations.length} pending invitation${res.locals.invitations.length === 1 ? "" : "s"}`)},
                });
            ...
            : html `
                              <div>
                                ...
                                <div class="dropdown--menu">
                                  $${res.locals.invitations.map((invitation) => html `
                                      <a
    ...
    app.locals.layouts.main = ({ req, res, head, showCourseSwitcher = true, body, }) => app.locals.layouts.application({
        req,
        res,
        head,
        showCourseSwitcher,
    ...
    app.locals.layouts.settings = ({ req, res, head, menuButton, menu, body }) => app.locals.layouts.application({
        req,
        res,
        head,
        extraHeaders: menu === html ``
            ? html ``
            : html `
              <div
                  ...
                  <button
                    ...
                    onload="${javascript `
                      (this.tooltip ??= tippy(this)).setProps({
                        trigger: "click",
                        interactive: true,
                        content: ${res.locals.html(html `<div class="dropdown--menu">$${menu}</div>`)},
                      });
                    `}"
                  >
                    $${menuButton}
  ...
  `;
    app.locals.partials.reportIssueHref = `mailto:${app.locals.options.administratorEmail}${qs.stringify({
    ...
    app.locals.helpers.Flash = {
        ...
        set({ req, res, theme, content }) {
            const flash = app.locals.database.get(sql `
        ...
        get({ req, res }) {
            if (req.cookies.flash === undefined)
                return undefined;
            const flash = app.locals.database.get(sql `SELECT "id", "theme", "content" FROM "flashes" WHERE "nonce" = ${req.cookies.flash}`);
    ...
    (async () => {
        while (true) {
            app.locals.database.run(sql `
          DELETE FROM "flashes"
          WHERE "createdAt" < ${new Date(Date.now() - app.locals.helpers.Flash.maxAge).toISOString()}
        `);
