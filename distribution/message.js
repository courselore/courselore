...
export default (app) => {
    app.locals.helpers.getMessage = ({ req, res, conversation, messageReference, }) => {
        const messageRow = app.locals.database.get(sql `
        SELECT "messages"."id",
               "messages"."createdAt",
               "messages"."updatedAt",
               "messages"."reference",
      ...
      `);
        ...
        const message = {
            ...
            authorEnrollment: messageRow.authorEnrollmentId !== null &&
                ...
                ? {
                    ...
                    user: {
                        ...
                        avatarlessBackgroundColor: messageRow.authorUserAvatarlessBackgroundColor,
                        biographySource: messageRow.authorUserBiographySource,
                        biographyPreprocessed: messageRow.authorUserBiographyPreprocessed,
                    },
            ...
            reading: messageRow.readingId === null ? null : { id: messageRow.readingId },
        };
        ...
        `)
            ...
            enrollment: reading.enrollmentId !== null &&
                reading.userId !== null &&
                ...
                ? {
                    ...
                    user: {
                        ...
                        avatarlessBackgroundColor: reading.userAvatarlessBackgroundColor,
        ...
        `)
            .map((like) => ({
            id: like.id,
            enrollment: like.enrollmentId !== null &&
                like.userId !== null &&
    ...
    app.locals.helpers.mayEditMessage = ({ req, res, message }) => res.locals.enrollment.courseRole === "staff" ||
        (message.authorEnrollment !== "no-longer-enrolled" &&
            message.authorEnrollment.id === res.locals.enrollment.id);
    app.get("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference/views", ...messageExistsMiddleware, (req, res) => {
        res.send(app.locals.layouts.partial({
            ...
            <div
              ...
              `)}"
            >
              $${res.locals.message.readings.reverse().map((reading) => html `
    ...
    app.post("/courses/:courseReference/conversations/:conversationReference/messages", ...app.locals.middlewares.isConversationAccessible, (req, res, next) => {
        if (![undefined, "on"].includes(req.body.isAnswer) ||
        ...
        const mostRecentMessage = app.locals.helpers.getMessage({
            ...
            res,
            conversation: res.locals.conversation,
            messageReference: String(res.locals.conversation.nextMessageReference - 1),
        });
        let processedContent;
        let messageReference;
        if (res.locals.conversation.type === "chat" &&
          ...
          `);
            ...
            UPDATE "conversations"
            SET "updatedAt" = ${new Date().toISOString()},
                "nextMessageReference" = ${res.locals.conversation.nextMessageReference + 1}
                $${res.locals.conversation.type === "question" &&
                res.locals.enrollment.courseRole === "staff" &&
                req.body.isAnswer === "on" &&
    ...
    app.patch("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference", ...messageExistsMiddleware, (req, res, next) => {
        ...
        if (typeof req.body.isAnswer === "string")
            if (!["true", "false"].includes(req.body.isAnswer) ||
                ...
                app.locals.database.run(sql `
              UPDATE "messages"
              SET "answerAt" = ${req.body.isAnswer === "true" ? new Date().toISOString() : null}
              WHERE "id" = ${res.locals.message.id}
            `);
        if (typeof req.body.isAnonymous === "string")
            if (!["true", "false"].includes(req.body.isAnonymous) ||
                ...
                app.locals.database.run(sql `
              UPDATE "messages"
              SET "anonymousAt" = ${req.body.isAnonymous === "true"
                    ? new Date().toISOString()
                    : null}
              WHERE "id" = ${res.locals.message.id}
            `);
    ...
    app.delete("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference", ...app.locals.middlewares.isCourseStaff, ...messageExistsMiddleware, (req, res, next) => {
        app.locals.database.run(sql `DELETE FROM "messages" WHERE "id" = ${res.locals.message.id}`);
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/conversations/${res.locals.conversation.reference}${qs.stringify({
            conversations: req.query.conversations,
            messages: req.query.messages,
        }, {
    ...
    app.post("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference/likes", ...messageExistsMiddleware, (req, res, next) => {
        if (res.locals.message.likes.some((like) => like.enrollment !== "no-longer-enrolled" &&
    ...
    app.delete("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference/likes", ...messageExistsMiddleware, (req, res, next) => {
        const like = res.locals.message.likes.find((like) => like.enrollment !== "no-longer-enrolled" &&
            like.enrollment.id === res.locals.enrollment.id);
        if (like === undefined)
            return next("validation");
        app.locals.database.run(sql `
    ...
    app.locals.helpers.mayEndorseMessage = ({ req, res, message }) => res.locals.enrollment.courseRole === "staff" &&
        res.locals.conversation.type === "question" &&
        message.reference !== "1" &&
        message.answerAt !== null &&
        (message.authorEnrollment === "no-longer-enrolled" ||
            message.authorEnrollment.courseRole !== "staff");
    const mayEndorseMessageMiddleware = [
    ...
    app.post("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference/endorsements", ...mayEndorseMessageMiddleware, (req, res, next) => {
        if (res.locals.message.endorsements.some((endorsement) => endorsement.enrollment !== "no-longer-enrolled" &&
    ...
    app.delete("/courses/:courseReference/conversations/:conversationReference/messages/:messageReference/endorsements", ...mayEndorseMessageMiddleware, (req, res, next) => {
        const endorsement = res.locals.message.endorsements.find((endorsement) => endorsement.enrollment !== "no-longer-enrolled" &&
            endorsement.enrollment.id === res.locals.enrollment.id);
        if (endorsement === undefined)
            return next("validation");
        app.locals.database.run(sql `DELETE FROM "endorsements" WHERE "id" = ${endorsement.id}`);
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/conversations/${res.locals.conversation.reference}${qs.stringify({
            conversations: req.query.conversations,
            messages: req.query.messages,
        }, {
    ...
    app.locals.mailers.notifications = ({ req, res, conversation, message, mentions, }) => {
        app.locals.database.run(sql `
      ...
      `);
        ...
        app.locals.database.executeTransaction(() => {
            const enrollments = app.locals.database.all(sql `
                  ...
                  <p>
                    <a
                      href="https://${app.locals.options.host}/courses/${res
                        .locals.course
                        .reference}/conversations/${conversation.reference}${qs.stringify({ messages: { messageReference: message.reference } }, {
                        addQueryPrefix: true,
                    })}"
                      >${message.authorEnrollment === "no-longer-enrolled"
                        ? "Someone who is no longer enrolled"
                        : message.anonymousAt !== null
                            ? `Anonymous ${enrollment.courseRole === "staff"
