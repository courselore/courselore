...
export default (app) => {
    app.locals.partials.user = ({ req, res, enrollment = undefined, user = enrollment === undefined
        ? undefined
        : enrollment === "no-longer-enrolled"
            ? "no-longer-enrolled"
            : enrollment.user, anonymous = user === undefined, avatar = true, decorate = user !== undefined, name = true, tooltip = name !== false, size = "sm", }) => {
        let userAvatar;
        let userName;
              ...
              >
                ...
                >
                  ${(() => {
                                ...
                                return `${nameParts[0][0]}${nameParts.length > 0
            ...
            <span
              ...
              onload="${javascript `
                ...
                (function update() {
                  ...
                  element.hidden = Date.now() - ${new Date(user.lastSeenOnlineAt).getTime()} > 5 * 60 * 1000;
        ...
        let userHTML = userAvatar !== undefined && userName !== undefined
      ...
      >`;
        ...
        let anonymousHTML = anonymousAvatar !== undefined && anonymousName !== undefined
    ...
    app.get("/settings", ...app.locals.middlewares.isSignedIn, (req, res) => {
        res.redirect(303, `https://${app.locals.options.host}/settings/profile`);
    });
    const userSettingsLayout = ({ req, res, head, body, }) => app.locals.layouts.settings({
        req,
        res,
        head,
        menuButton: html `
      ...
      `,
        ...
        <a
          ...
          class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/profile")
            ? "button--blue"
            : "button--transparent"}"
        >
        ...
        <a
          ...
          class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/email-and-password")
            ? "button--blue"
            : "button--transparent"}"
        >
          <i
        ...
        <a
          ...
          class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/notifications")
            ? "button--blue"
            : "button--transparent"}"
        >
          <i
    ...
    app.get("/settings/profile", ...app.locals.middlewares.isSignedIn, (req, res) => {
        res.send(userSettingsLayout({
            req,
            res,
                  ...
                  <input
                    ...
                    onload="${javascript `
                      ...
                      this.upload = async (fileList) => {
                        ...
                        const response = await fetch("https://${app.locals.options.host}/settings/profile/avatar", {
                          method: "POST",
                          body,
                        });
    ...
    app.patch("/settings/profile", ...app.locals.middlewares.isSignedIn, (req, res, next) => {
        ...
        app.locals.database.run(sql `
          ...
          SET "name" = ${req.body.name},
              ...
              "avatar" = ${req.body.avatar.trim() === "" ? null : req.body.avatar},
              "biographySource" = ${req.body.biography.trim() === "" ? null : req.body.biography},
              "biographyPreprocessed" = ${req.body.biography.trim() === ""
            ? null
            : app.locals.partials.content({
                req,
                res,
    ...
    app.post("/settings/profile/avatar", asyncHandler(async (req, res, next) => {
        ...
        await req.files.avatar.mv(path.join(app.locals.options.dataDirectory, `files/${folder}/${name}`));
        const ext = path.extname(name);
        const nameAvatar = `${name.slice(0, name.length - ext.length)}--avatar${ext}`;
        try {
            await sharp(req.files.avatar.data, { limitInputPixels: false })
                ...
                .toFile(path.join(app.locals.options.dataDirectory, `files/${folder}/${nameAvatar}`));
        }
        catch (error) {
            return next("validation");
        }
        res.send(`https://${app.locals.options.host}/files/${folder}/${encodeURIComponent(nameAvatar)}`);
    }), ((err, req, res, next) => {
        if (err === "validation")
            return res
                .status(422)
                .send(`Something went wrong in uploading your avatar. Please report to the system administrator at ${app.locals.options.administratorEmail}.`);
        next(err);
    }));
    app.get("/settings/email-and-password", ...app.locals.middlewares.isSignedIn, (req, res) => {
        res.send(userSettingsLayout({
            req,
            res,
            head: html `<title>
    ...
    app.patch("/settings/email-and-password", ...app.locals.middlewares.isSignedIn, asyncHandler(async (req, res, next) => {
        if (typeof req.body.currentPassword !== "string" ||
            req.body.currentPassword.trim() === "")
            return next("validation");
        if (!(await argon2.verify(res.locals.user.password, req.body.currentPassword))) {
            app.locals.helpers.Flash.set({
                req,
                res,
                theme: "rose",
            ...
            return res.redirect(303, `https://${app.locals.options.host}/settings/email-and-password`);
        }
        ...
        if (typeof req.body.newPassword === "string") {
            ...
            app.locals.database.run(sql `
            UPDATE "users"
            SET "password" =  ${await argon2.hash(req.body.newPassword, app.locals.options.argon2)}
            WHERE "id" = ${res.locals.user.id}
          `);
        ...
        res.redirect(303, `https://${app.locals.options.host}/settings/email-and-password`);
    }));
    app.get("/settings/notifications", ...app.locals.middlewares.isSignedIn, (req, res) => {
        res.send(userSettingsLayout({
            req,
            res,
            ...
            >
              ...
              $${(() => {
                const disabled = res.locals.user.emailNotificationsForAllMessagesAt === null &&
                    ...
                    res.locals.user
                        .emailNotificationsForMessagesInConversationsYouStartedAt ===
                        null;
                const isEmailNotificationsDigests = res.locals.user.emailNotificationsDigestsFrequency !== null ||
                    disabled;
                return html `
                  <div
    ...
    app.patch("/settings/notifications", ...app.locals.middlewares.isSignedIn, (req, res, next) => {
        if (![undefined, "on"].includes(req.body.isEmailNotificationsForAllMessages) ||
            ![undefined, "on"].includes(req.body.isEmailNotificationsForMentions) ||
            ![undefined, "on"].includes(req.body
                .isEmailNotificationsForMessagesInConversationsInWhichYouParticipated) ||
            ![undefined, "on"].includes(req.body.isEmailNotificationsForMessagesInConversationsYouStarted) ||
                ...
                (typeof req.body.isEmailNotificationsDigests !== "string" ||
                    ...
                    (req.body.isEmailNotificationsDigests === "true" &&
                        (typeof req.body.emailNotificationsDigestsFrequency !==
                            "string" ||
                            !userEmailNotificationsDigestsFrequencies.includes(req.body.emailNotificationsDigestsFrequency))))))
            return next("validation");
        app.locals.database.run(sql `
          UPDATE "users"
          SET "emailNotificationsForAllMessagesAt" = ${req.body.isEmailNotificationsForAllMessages === "on"
            ? new Date().toISOString()
            : null},
              "emailNotificationsForMentionsAt" = ${req.body.isEmailNotificationsForMentions === "on"
            ? new Date().toISOString()
            : null},
              "emailNotificationsForMessagesInConversationsInWhichYouParticipatedAt" = ${req.body
            .isEmailNotificationsForMessagesInConversationsInWhichYouParticipated ===
            "on"
            ? new Date().toISOString()
            : null},
              "emailNotificationsForMessagesInConversationsYouStartedAt" = ${req.body
            .isEmailNotificationsForMessagesInConversationsYouStarted ===
            "on"
            ? new Date().toISOString()
            : null},
              "emailNotificationsDigestsFrequency" = ${req.body.isEmailNotificationsDigests === "true"
       ...
       `);
        ...
        res.redirect(303, `https://${app.locals.options.host}/settings/notifications`);
    });
};
