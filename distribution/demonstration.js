...
import { userAvatarlessBackgroundColors, userEmailNotificationsDigestsFrequencies, courseRoles, enrollmentAccentColors, conversationTypes, } from "./index.js";
export default (app) => {
    ...
    const handler = asyncHandler(async (req, res) => {
        ...
        const users = lodash.times(151, (userIndex) => {
            ...
            const isEmailNotificationsForMentions = !isEmailNotificationsForNone && Math.random() < 0.8;
            const isEmailNotificationsForMessagesInConversationsInWhichYouParticipated = !isEmailNotificationsForNone && Math.random() < 0.8;
            const isEmailNotificationsForMessagesInConversationsYouStarted = isEmailNotificationsForMessagesInConversationsInWhichYouParticipated ||
                (!isEmailNotificationsForNone && Math.random() < 0.8);
            return app.locals.database.get(sql `
          INSERT INTO "users" (
            "createdAt",
            "lastSeenOnlineAt",
            "reference",
          ...
          VALUES (
            ...
            ${new Date(Date.now() -
                (Math.random() < 0.5 ? 0 : lodash.random(0, 5 * 60 * 60 * 1000))).toISOString()},
            ...
            ${html `${name}`},
            ${Math.random() < 0.6
                ? `https://${app.locals.options.host}/node_modules/fake-avatars/avatars/${avatarIndices.shift()}.png`
                : null},
...
`;
        ...
        for (const { name, code, courseRole: courseRole, accentColor, enrollmentsUsers, isArchived, } of [
            {
          ...
          RETURNING *
        `);
            const enrollment = app.locals.database.get(sql `
        ...
        `);
            for (const _ of lodash.times(20)) {
                const expiresAt = Math.random() < 0.3
                    ? new Date(Date.now() +
                        lodash.random(-30 * 24 * 60 * 60 * 1000, 30 * 24 * 60 * 60 * 1000)).toISOString()
                    : null;
            ...
            VALUES (
              ...
              ${user === null || Math.random() < 0.4
                    ...
                    : new Date((expiresAt === null
                        ...
                        lodash.random(20 * 24 * 60 * 60 * 1000)).toISOString()},
              ${course.id},
              ${cryptoRandomString({ length: 10, type: "numeric" })},
              ${user?.email},
          ...
          `);
            ...
            const enrollments = [
                enrollment,
                ...enrollmentsUsers.map((enrollmentUser) => app.locals.database.get(sql `
                INSERT INTO "enrollments" ("createdAt", "user", "course", "reference", "courseRole", "accentColor")
                VALUES (
                  ${new Date().toISOString()},
                  ${enrollmentUser.id},
            ...
            const staff = enrollments.filter((enrollment) => enrollment.courseRole === "staff");
            const tags = [
            ...
            ].map(({ name, staffOnlyAt }) => app.locals.database.get(sql `
            ...
            for (let conversationReference = 2; conversationReference < course.nextConversationReference; conversationReference++)
                conversationCreatedAts.unshift(new Date(new Date(conversationCreatedAts[0]).getTime() -
                    lodash.random(6 * 60 * 60 * 1000, 2 * 24 * 60 * 60 * 1000)).toISOString());
            for (let conversationReference = 1; conversationReference < course.nextConversationReference; conversationReference++) {
                const conversationCreatedAt = conversationCreatedAts[conversationReference - 1];
                const type = conversationReference === 1
                    ? conversationTypes[1]
                    : conversationTypes[Math.random() < 0.5 ? 0 : Math.random() < 0.8 ? 1 : 2];
                const nextMessageReference = type === "chat" ? lodash.random(50, 100) : lodash.random(2, 30);
                const messageCreatedAts = [conversationCreatedAt];
                for (let messageReference = 1; messageReference < nextMessageReference; messageReference++)
                    messageCreatedAts.push(new Date(Math.min(Date.now(), new Date(messageCreatedAts[messageCreatedAts.length - 1]).getTime() + lodash.random(12 * 60 * 60 * 1000))).toISOString());
                const title = conversationReference === 1
                    ? `Example of All Features in Rich-Text Messages`
                    : `${lodash.capitalize(casual.words(lodash.random(3, 9)))}${type === "question" ? "?" : ""}`;
                const conversationAuthorEnrollment = lodash.sample(enrollments);
                const conversation = app.locals.database.get(sql `
          ...
          `);
                for (let messageReference = 1; messageReference < nextMessageReference; messageReference++) {
                    ...
                    const contentSource = conversationReference === 1 && messageReference === 1
                    ...
                    const messageAuthorEnrollment = messageReference === 1
              ...
              VALUES (
                ...
                ${Math.random() < 0.8
                        ? null
                        : new Date(Math.min(Date.now(), new Date(messageCreatedAt).getTime() +
                            lodash.random(5 * 60 * 60 * 1000, 18 * 60 * 60 * 1000))).toISOString()},
                ${conversation.id},
              ...
              RETURNING *
            `);
                    const readers = conversation.staffOnlyAt === null ? enrollments : staff;
                    let readingCreatedAt = messageCreatedAt;
                    for (const enrollment of lodash.sampleSize(readers, lodash.random(1, readers.length))) {
                        readingCreatedAt = new Date(Math.min(Date.now(), new Date(readingCreatedAt).getTime() +
                            lodash.random(12 * 60 * 60 * 1000))).toISOString();
                        app.locals.database.run(sql `
              ...
              `);
                    ...
                    for (const enrollment of lodash.sampleSize(staff, Math.random() < 0.8 ? 0 : lodash.random(2)))
                        app.locals.database.run(sql `
              ...
              `);
                    for (const enrollment of lodash.sampleSize(enrollments, Math.random() < (conversation.type === "chat" ? 0.9 : 0.5)
                        ? 0
                        : lodash.random(5)))
                        app.locals.database.run(sql `
    ...
    app.post("/demonstration-data", ...app.locals.middlewares.isSignedOut, handler);
    app.post("/demonstration-data", ...app.locals.middlewares.isSignedIn, handler);
};
