...
export default async (app) => {
    ...
    app.locals.database = new Database(path.join(app.locals.options.dataDirectory, "courselore.db"), process.env.LOG_DATABASE === "true" ? { verbose: console.log } : undefined);
    app.locals.database.pragma("journal_mode = WAL");
    await app.locals.database.migrate(sql `
    ...
    `, () => {
        const makeMessageReferenceInMessagePermanentLinkVisibleToServerForPaginationToWork = (text) => text.replace(new RegExp(`(?<=https://${escapeStringRegexp(app.locals.options.host)}/courses/\\d+/conversations/\\d+)#message--(?=\\d+)`, "gi"), "?messageReference=");
        for (const user of app.locals.database.all(sql `
          SELECT "id", "biographySource", "biographyPreprocessed"
          FROM "users"
          ORDER BY "id"
        `))
            if (user.biographySource !== null &&
                user.biographyPreprocessed !== null)
                app.locals.database.run(sql `
              UPDATE "users"
              SET "biographySource" = ${makeMessageReferenceInMessagePermanentLinkVisibleToServerForPaginationToWork(user.biographySource)},
                  "biographyPreprocessed" = ${makeMessageReferenceInMessagePermanentLinkVisibleToServerForPaginationToWork(user.biographyPreprocessed)}
              WHERE "id" = ${user.id}
            `);
        for (const message of app.locals.database.all(sql `
        ...
        `))
            app.locals.database.run(sql `
            UPDATE "messages"
            SET "contentSource" = ${makeMessageReferenceInMessagePermanentLinkVisibleToServerForPaginationToWork(message.contentSource)},
                "contentPreprocessed" = ${makeMessageReferenceInMessagePermanentLinkVisibleToServerForPaginationToWork(message.contentPreprocessed)},
                "contentSearch" = ${makeMessageReferenceInMessagePermanentLinkVisibleToServerForPaginationToWork(message.contentSearch)}
            WHERE "id" = ${message.id}
          `);
    }, sql `
      DROP TABLE "flashes";
    ...
    `, () => {
        const changeMessageReferencePermanentLinkQueryParameter = (text) => text.replace(new RegExp(`(?<=https://${escapeStringRegexp(app.locals.options.host)}/courses/\\d+/conversations/\\d+)\\?messageReference=(?=\\d+)`, "gi"), "?messages%5BmessageReference%5D=");
        for (const user of app.locals.database.all(sql `
          SELECT "id", "biographySource", "biographyPreprocessed"
          FROM "users"
          ORDER BY "id"
        `))
            if (user.biographySource !== null &&
                ...
                app.locals.database.run(sql `
              UPDATE "users"
              SET "biographySource" = ${changeMessageReferencePermanentLinkQueryParameter(user.biographySource)},
                  "biographyPreprocessed" = ${changeMessageReferencePermanentLinkQueryParameter(user.biographyPreprocessed)}
              WHERE "id" = ${user.id}
            `);
        for (const message of app.locals.database.all(sql `
          SELECT "id", "contentSource", "contentPreprocessed", "contentSearch"
        ...
        `))
            app.locals.database.run(sql `
            UPDATE "messages"
            SET "contentSource" = ${changeMessageReferencePermanentLinkQueryParameter(message.contentSource)},
                "contentPreprocessed" = ${changeMessageReferencePermanentLinkQueryParameter(message.contentPreprocessed)},
                "contentSearch" = ${changeMessageReferencePermanentLinkQueryParameter(message.contentSearch)}
            WHERE "id" = ${message.id}
          `);
    }, sql `
    ...
    `, () => {
        ...
        while (true) {
            const user = (await prompts({
                type: "autocomplete",
                name: "output",
                message: "Courselore 4.0.0 introduces an administration interface and the role of system administrators. Please select a user to become the first administrator.",
