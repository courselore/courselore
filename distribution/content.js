...
import rehypeSanitize, { defaultSchema as rehypeSanitizeDefaultSchema, } from "rehype-sanitize";
...
export default async (app) => {
    app.locals.partials.content = await (async () => {
        const unifiedProcessor = unified()
            ...
            .use(rehypeRaw)
            .use(rehypeSanitize, deepMerge(rehypeSanitizeDefaultSchema, {
            attributes: {
                code: ["className"],
            ...
            .use(rehypeStringify);
        return ({ req, res, type, content, decorate = false, search = undefined, }) => {
      ...
      `).firstElementChild;
            const contentPreprocessed = type === "source" ? contentElement.innerHTML : undefined;
            const contentSearch = type === "source" ? contentElement.textContent : undefined;
            let mentions;
            for (const element of contentElement.querySelectorAll("li, td, th, dt, dd"))
                element.innerHTML = [...element.childNodes].some((node) => node.nodeType === node.TEXT_NODE && node.textContent.trim() !== "")
            ...
            for (const element of contentElement.querySelectorAll("img")) {
                ...
                if (!element
                    ...
                    element.setAttribute("src", `https://${app.locals.options.host}/content/image-proxy${qs.stringify({ url: element.getAttribute("src") }, { addQueryPrefix: true })}`);
            }
            for (const element of contentElement.querySelectorAll("details")) {
                ...
                switch (summaries.length) {
                    case 0:
                        summaries.push(JSDOM.fragment(html `<summary>See More</summary>`)
                            .firstElementChild);
                        break;
                    case 1:
                        break;
            ...
            for (const element of contentElement.querySelectorAll("[href]")) {
                ...
                if (href.startsWith("#user-content-user-content-fnref-") &&
                    element.innerHTML === "↩")
                ...
                if ((!href.startsWith("#") &&
                    ...
                    element.setAttribute("onload", javascript `
              ${href.startsWith(`https://${app.locals.options.host}/files/`)
                        ? javascript ``
                        : javascript `
                      (this.tooltip ??= tippy(this)).setProps({
                        touch: false,
                        content: ${res.locals.html(html `External link to
                ...
                if (res.locals.course !== undefined) {
                    ...
                    for (const element of contentElement.querySelectorAll("a")) {
                        ...
                        if (href !== element.textContent.trim())
                            continue;
                        const match = href.match(new RegExp(`^https://${escapeStringRegexp(app.locals.options.host)}/courses/(\\d+)/conversations/(\\d+)(?:\\?messages%5BmessageReference%5D=(\\d+))?$`));
                        if (match === null)
                            continue;
                        const [courseReference, conversationReference, messageReference] = match.slice(1);
                        if (courseReference !== res.locals.course.reference)
                            continue;
                    ...
                    (function processTree(node) {
                        ...
                        function processNode() {
                            switch (node.nodeType) {
                                case node.TEXT_NODE:
                                    ...
                                    newNodeHTML = newNodeHTML.replace(/(?<!\w)@(everyone|staff|students|anonymous|[0-9a-z-]+)(?!\w)/gi, (match, mention) => {
                          ...
                          >`;
                                            ...
                                            default:
                                                ...
                                                const enrollmentRow = app.locals.database.get(sql `
                            ...
                            `);
                                                ...
                                                const enrollment = {
                                                    ...
                                                    user: {
                                                        ...
                                                        avatarlessBackgroundColor: enrollmentRow.userAvatarlessBackgroundColor,
                                                        biographySource: enrollmentRow.userBiographySource,
                                                        biographyPreprocessed: enrollmentRow.userBiographyPreprocessed,
                                                    },
                      ...
                      >`;
                                    ...
                                    newNodeHTML = newNodeHTML.replace(/(?<!\w)#(\d+)(?:\/(\d+))?(?!\w)/g, (match, conversationReference, messageReference) => {
                                        const conversation = app.locals.helpers.getConversation({
                                            req: narrowReq,
                                            res: narrowRes,
                                            conversationReference,
                    ...
                    for (const element of contentElement.querySelectorAll("a")) {
                        ...
                        if (href === null)
                            continue;
                        const hrefMatch = href.match(new RegExp(`^https://${escapeStringRegexp(app.locals.options.host)}/courses/(\\d+)/conversations/(\\d+)(?:\\?messages%5BmessageReference%5D=(\\d+))?$`));
                        if (hrefMatch === null)
                            continue;
                        const [hrefCourseReference, hrefConversationReference, hrefMessageReference,] = hrefMatch.slice(1);
                        if (hrefCourseReference !== res.locals.course.reference)
                            continue;
                        ...
                        if (textContentMatch === null)
                            continue;
                        const [textContentConversationReference, textContentMessageReference,] = textContentMatch.slice(1);
                        if (hrefConversationReference !== textContentConversationReference ||
                            ...
                            switch (node.nodeType) {
                                case node.TEXT_NODE:
                                    ...
                                    if (node.textContent === null || parentElement === null)
                                        return;
                                    parentElement.replaceChild(JSDOM.fragment(app.locals.helpers.highlightSearchResult(html `${node.textContent}`, search)), node);
                                    break;
                            }
                        }
                    })(contentElement);
    ...
    app.get("/content/image-proxy", asyncHandler(async (req, res) => {
        if (typeof req.query.url !== "string" ||
            !["http://", "https://"].some((urlPrefix) => req.query.url.toLowerCase().startsWith(urlPrefix)))
            return res.status(422).end();
        await stream.pipeline(got
    ...
    app.locals.partials.contentEditor = ({ req, res, name = "content", contentSource = "", required = true, compact = false, }) => html `
    <div
      key="content-editor"
                  ...
                  onload="${javascript `
                    ...
                    this.onclick = async (event) => {
                      ...
                      leafac.loadPartial(
                        preview,
                        await (
                          await fetch(${JSON.stringify(`https://${app.locals.options.host}${res.locals.course === undefined
            ? ""
            : `/courses/${res.locals.course.reference}`}/content-editor/preview`)}, {
                            method: "POST",
      ...
      >
              ...
              <input
                ...
                onload="${javascript `
                  ...
                  this.upload = async (fileList) => {
                    ...
                    const response = await (await fetch(${JSON.stringify(`https://${app.locals.options.host}/content-editor/attachments`)}, {
                      method: "POST",
                      body,
                    })).text();
                  ...
                  const checkIsSignedIn = ${res.locals.user === undefined
        ? javascript `
                          (() => {
        ...
        ? javascript `
                        ...
                        const dropdownMenus = [
                          {
                            trigger: "@",
                            route: ${JSON.stringify(`https://${app.locals.options.host}/courses/${res.locals.course.reference}/${res.locals.conversation !== undefined
            ...
            : ``}content-editor/mention-user-search`)},
                          ...
                          {
                            trigger: "#",
                            route: ${JSON.stringify(`https://${app.locals.options.host}/courses/${res.locals.course.reference}/content-editor/refer-to-conversation-or-message-search`)},
...
${contentSource}</textarea
  ...
  `;
    (() => {
        const handler = (req, res, next) => {
            ...
            JOIN "usersNameSearchIndex" ON "users"."id" = "usersNameSearchIndex"."rowid" AND
                                          "usersNameSearchIndex" MATCH ${app.locals.helpers.sanitizeSearch(req.query.search, { prefix: true })}
          ...
          `)
            ...
            res.send(app.locals.layouts.partial({
                req,
                res,
                body: html `
            $${enrollments.length === 0
                ...
                `
                    : enrollments.map((enrollment) => html `
                    <button
                      ...
                      onload="${javascript `
                        this.onclick = () => {
                          this.closest(".content-editor").querySelector(".content-editor--write--textarea").dropdownMenuComplete(${JSON.stringify(`${enrollment.reference}--${slugify(enrollment.user.name)}`)});  
                        };
                    `}"
                    >
        ...
        app.get("/courses/:courseReference/content-editor/mention-user-search", ...app.locals.middlewares.isEnrolledInCourse, handler);
        app.get("/courses/:courseReference/conversations/:conversationReference/content-editor/mention-user-search", ...app.locals.middlewares.isConversationAccessible, handler);
    })();
    app.get("/courses/:courseReference/content-editor/refer-to-conversation-or-message-search", ...app.locals.middlewares.isEnrolledInCourse, (req, res, next) => {
        if (typeof req.query.search !== "string" ||
            req.query.search.trim() === "")
            return next("validation");
        let results = html ``;
        if (req.query.search.match(/^\d+$/) !== null)
            for (const conversationRow of app.locals.database.all(sql `
            SELECT "conversations"."reference"
            FROM "conversations"
            JOIN "conversationsReferenceIndex" ON "conversations"."id" = "conversationsReferenceIndex"."rowid" AND
                                                  "conversationsReferenceIndex" MATCH ${app.locals.helpers.sanitizeSearch(req.query.search, { prefix: true })}
          ...
          `)) {
            ...
            <button
              ...
              onload="${javascript `
                this.onclick = () => {
                  this.closest(".content-editor").querySelector(".content-editor--write--textarea").dropdownMenuComplete(${JSON.stringify(conversation.reference)});
                };
              `}"
            >
              <span>
                <span class="secondary">
                  $${app.locals.helpers.highlightSearchResult(`#${conversation.reference}`, `#${req.query.search}`, { prefix: true })}
                </span>
        ...
        const messageReferenceSearchMatch = req.query.search.match(/^(\d+)\/(\d*)$/);
        if (messageReferenceSearchMatch !== null) {
            const [conversationReference, messageReferenceSearch] = messageReferenceSearchMatch.slice(1);
            const conversation = app.locals.helpers.getConversation({
                req,
                res,
                conversationReference,
            ...
            if (conversation !== undefined) {
                for (const messageRow of app.locals.database.all(sql `
              ...
              $${messageReferenceSearch === ""
                    ...
                    JOIN "messagesReferenceIndex" ON "messages"."id" = "messagesReferenceIndex"."rowid" AND
                                                      "messagesReferenceIndex" MATCH ${app.locals.helpers.sanitizeSearch(messageReferenceSearch, { prefix: true })}
                  `}
            ...
            `)) {
              ...
              <button
                ...
                onload="${javascript `
                  this.onclick = () => {
                    this.closest(".content-editor").querySelector(".content-editor--write--textarea").dropdownMenuComplete(${JSON.stringify(`${conversation.reference}/${message.reference}`)});
                  };
                `}"
              >
                <div>
                  <div>
                    <span class="secondary">
                      $${app.locals.helpers.highlightSearchResult(`#${conversation.reference}/${message.reference}`, `#${req.query.search}`, { prefix: true })}
                    </span>
                    <span class="strong">${conversation.title}</span>
                  </div>
                  <div class="secondary">
            ...
            >
              <span>
                <span class="secondary">
                  $${app.locals.helpers.highlightSearchResult(`#${conversation.reference}`, `#${conversationReference}`)}
                </span>
                <span class="strong">${conversation.title}</span>
              </span>
            </button>
        ...
        for (const conversationRow of app.locals.database.all(sql `
          ...
          JOIN "conversationsTitleSearchIndex" ON "conversations"."id" = "conversationsTitleSearchIndex"."rowid" AND
                                                  "conversationsTitleSearchIndex" MATCH ${app.locals.helpers.sanitizeSearch(req.query.search, { prefix: true })}
          WHERE "conversations"."course" = ${res.locals.course.id}
        ...
        for (const messageRow of app.locals.database.all(sql `
          ...
          JOIN "messagesContentSearchIndex" ON "messages"."id" = "messagesContentSearchIndex"."rowid" AND
                                                "messagesContentSearchIndex" MATCH ${app.locals.helpers.sanitizeSearch(req.query.search, { prefix: true })}
          JOIN "conversations" ON "messages"."conversation" = "conversations"."id" AND
                                  "conversations"."course" = ${res.locals.course.id}
    ...
    app.post("/content-editor/attachments", ...app.locals.middlewares.isSignedIn, asyncHandler(async (req, res, next) => {
        ...
        for (const attachment of attachments) {
            ...
            await attachment.mv(path.join(app.locals.options.dataDirectory, `files/${folder}/${attachment.name}`));
            const href = `https://${app.locals.options.host}/files/${folder}/${encodeURIComponent(attachment.name)}`;
            if (attachment.mimetype.startsWith("image/"))
                try {
                    ...
                    if (metadata.width <= maximumWidth) {
                        attachmentsContentSources += `[<img src="${href}" alt="${attachment.name}" width="${metadata.width / 2}" />](${href})\n\n`;
                        continue;
                    }
                    const ext = path.extname(attachment.name);
                    const nameThumbnail = `${attachment.name.slice(0, attachment.name.length - ext.length)}--thumbnail${ext}`;
                    await image
                        .rotate()
                        .resize({ width: maximumWidth })
                        .toFile(path.join(app.locals.options.dataDirectory, `files/${folder}/${nameThumbnail}`));
                    attachmentsContentSources += `[<img src="https://${app.locals.options.host}/files/${folder}/${encodeURIComponent(nameThumbnail)}" alt="${attachment.name}" width="${maximumWidth / 2}" />](${href})\n\n`;
                    continue;
                }
                catch { }
    ...
    (() => {
        ...
        app.post("/courses/:courseReference/content-editor/preview", ...app.locals.middlewares.isEnrolledInCourse, handler);
        app.post("/content-editor/preview", ...app.locals.middlewares.isSignedIn, handler);
        app.post("/content-editor/preview", ...app.locals.middlewares.isSignedOut, handler);
    })();
};
