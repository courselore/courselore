...
export const systemRoles = ["none", "staff", "administrator"];
export default (app) => {
    app.locals.options = {
        ...app.locals.options,
    ...
    if (app.locals.options.environment === "production")
        (async () => {
            while (true) {
                try {
                    const latestVersion = (await got("https://api.github.com/repos/courselore/courselore/releases/latest").json()).tag_name.replace(/^v/, "");
                    if (latestVersion !== app.locals.options.version) {
                        app.locals.options.latestVersion = latestVersion;
                        console.log(`${new Date().toISOString()}\tUPDATE CHECK\tNew version available: ${app.locals.options.latestVersion}.`);
                    }
                    else
                        console.log(`${new Date().toISOString()}\tUPDATE CHECK\tCurrent version is the latest.`);
                }
                catch (error) {
                    console.error(`${new Date().toISOString()}\tUPDATE CHECK\tERROR:\n${error}`);
                }
    ...
    app.get("/administration", ...isAdministratorMiddleware, (res, req) => {
        req.redirect(303, `https://${app.locals.options.host}/administration/system-settings`);
    });
    const administrationLayout = ({ req, res, head, body, }) => app.locals.layouts.settings({
        req,
        res,
        head,
        menuButton: html `
      ...
      `,
        ...
        <a
          ...
          class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/administration/system-settings")
            ? "button--blue"
            : "button--transparent"}"
        >
        ...
        <a
          ...
          class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/administration/users")
            ? "button--blue"
            : "button--transparent"}"
        >
          <i
    ...
    app.get("/administration/system-settings", ...isAdministratorMiddleware, (req, res) => {
        res.send(administrationLayout({
            req,
            res,
            head: html `
    ...
    app.patch("/administration/system-settings", ...isAdministratorMiddleware, (req, res, next) => {
        if (typeof req.body.userSystemRolesWhoMayCreateCourses !== "string" ||
            !userSystemRolesWhoMayCreateCourseses.includes(req.body.userSystemRolesWhoMayCreateCourses))
            return next("validation");
        const administrationOptions = app.locals.database.get(sql `
        ...
        res.redirect(303, `https://${app.locals.options.host}/administration/system-settings`);
    });
    ...
    app.get("/administration/users", ...isAdministratorMiddleware, (req, res) => {
        const users = app.locals.database.all(sql `
        SELECT "id",
                "lastSeenOnlineAt",
                "reference",
                "email",
      ...
      `);
        res.send(administrationLayout({
          ...
          $${users.map((user) => {
                ...
                const isOnlyAdministrator = isSelf &&
              ...
              >
                ...
                >
                  <div>
                    <div
                      data-filterable-phrases="${JSON.stringify(app.locals.helpers.splitFilterablePhrases(user.name))}"
                      class="strong"
                    >
                      ${user.name}
                    </div>
                    <div class="secondary">
                      <span
                        data-filterable-phrases="${JSON.stringify(app.locals.helpers.splitFilterablePhrases(user.email))}"
                        css="${res.locals.css(css `
                      ...
                      <button
                        ...
                        onload="${javascript `
                          ...
                          this.onclick = async () => {
                            await navigator.clipboard.writeText(${JSON.stringify(user.email)});
                            this.copied.show();
                    ...
                    >
                      <span>
                        Last seen online
                        <time
                          datetime="${new Date(user.lastSeenOnlineAt).toISOString()}"
                          onload="${javascript `
                  ...
                  >
                    ...
                    >
                      <button
                        class="button button--tight button--tight--inline button--transparent ${systemRoleTextColor[user.systemRole]}"
                        onload="${javascript `
                          (this.tooltip ??= tippy(this)).setProps({
                            touch: false,
                          ...
                          (this.dropdown ??= tippy(this)).setProps({
                            ...
                            content: ${res.locals.html(html `
                                <div class="dropdown--menu">
                                  $${systemRoles.map((systemRole) => html `
                                        <form
                                          key="role--${systemRole}"
                                          method="PATCH"
                                          action="${action}"
                    ...
                    user.systemRole
                    ? "button--blue"
                    : "button--transparent"} ${systemRoleTextColor[systemRole]}"
                                              $${isOnlyAdministrator
                    ? html `
                                                    type="button"
                                                    onload="${javascript `
    ...
    app.patch("/users/:userReference", ...isAdministratorMiddleware, (req, res, next) => {
        const managedUser = app.locals.database.get(sql `
        SELECT "id"
        FROM "users"
        WHERE "reference" = ${req.params.userReference}
      `);
        if (managedUser === undefined)
            return next("route");
        ...
        if (typeof req.body.role === "string") {
            if (!systemRoles.includes(req.body.role))
                return next("validation");
            app.locals.database.run(sql `UPDATE "users" SET "systemRole" = ${req.body.role} WHERE "id" = ${managedUser.id}`);
        }
        app.locals.helpers.Flash.set({
            req,
            res,
