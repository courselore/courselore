...
export const courseRoles = ["student", "staff"];
...
export default (app) => {
    ...
    app.locals.partials.course = ({ req, res, course, enrollment = undefined, tight = false, }) => html `
    <div
  ...
  `;
    app.locals.partials.courses = ({ req, res, tight = false }) => {
        let courses = html ``;
        const [unarchived, archived] = lodash.partition(res.locals.enrollments, (enrollment) => enrollment.course.archivedAt === null);
        if (unarchived.length > 0)
            courses += html `
        $${unarchived.map((enrollment) => html `
              <a
  ...
  `;
    app.get("/", ...app.locals.middlewares.isSignedIn, (req, res) => {
        switch (res.locals.enrollments.length) {
            case 0:
                res.send(app.locals.layouts.main({
                    req,
                    res,
              ...
              `,
                ...
                break;
            case 1:
                res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.enrollments[0].course.reference}`);
                break;
            default:
                res.send(app.locals.layouts.main({
                    req,
    ...
    const mayCreateCoursesMiddleware = [
        ...app.locals.middlewares.isSignedIn,
        (req, res, next) => {
            if (res.locals.mayCreateCourses)
                return next();
            next("route");
        },
    ];
    app.get("/courses/new", ...mayCreateCoursesMiddleware, (req, res) => {
        res.send(app.locals.layouts.main({
            req,
            res,
    ...
    app.post("/courses", ...mayCreateCoursesMiddleware, (req, res, next) => {
        ...
        const course = app.locals.database.get(sql `
        INSERT INTO "courses" (
          "createdAt",
          "reference",
          "name",
      ...
      `);
        res.redirect(303, `https://${app.locals.options.host}/courses/${course.reference}`);
    });
    const defaultAccentColor = ({ req, res, }) => {
        const accentColorsInUse = new Set(res.locals.enrollments.map((enrollment) => enrollment.accentColor));
        const accentColorsAvailable = new Set(enrollmentAccentColors);
        for (const accentColorInUse of accentColorsInUse) {
            accentColorsAvailable.delete(accentColorInUse);
            if (accentColorsAvailable.size === 1)
                break;
        }
    ...
    app.locals.middlewares.isEnrolledInCourse = [
        ...app.locals.middlewares.isSignedIn,
        (req, res, next) => {
            const enrollment = res.locals.enrollments.find((enrollment) => enrollment.course.reference === req.params.courseReference);
            if (enrollment === undefined)
                return next("route");
            res.locals.enrollment = enrollment;
            res.locals.course = enrollment.course;
            res.locals.conversationsCount = app.locals.database.get(sql `
          ...
          $${res.locals.enrollment.courseRole !== "staff"
                ...
                : sql ``}
        `).count;
            res.locals.tags = app.locals.database.all(sql `
          ...
          ORDER BY "id" ASC
        `);
            if (res.locals.course.archivedAt !== null &&
          ...
          `,
                ...
                return res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}`);
            }
            next();
        },
    ];
    ...
    app.get("/courses/:courseReference", ...app.locals.middlewares.isEnrolledInCourse, ...app.locals.middlewares.liveUpdates, (req, res) => {
        if (res.locals.conversationsCount === 0)
            return res.send(app.locals.layouts.main({
                req,
                res,
              ...
              >
                ...
                <div class="menu-box">
                  ...
                  <a
                    ...
                    newConversation: {
                        type: res.locals.enrollment.courseRole === "staff"
                            ? "note"
                            : "question",
                    },
                }, { addQueryPrefix: true })}"
    ...
    app.get("/courses/:courseReference/settings", ...app.locals.middlewares.isEnrolledInCourse, (req, res) => {
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/settings/${res.locals.enrollment.courseRole === "staff"
            ? "course-information"
            : "your-enrollment"}`);
    });
    const courseSettingsLayout = ({ req, res, head, body, }) => app.locals.layouts.settings({
        req,
        res,
        head,
        menuButton: html `
        ...
        Course Settings
      `,
        menu: res.locals.enrollment.courseRole === "staff"
            ? html `
              <a
                href="https://${app.locals.options.host}/courses/${res.locals
                .course.reference}/settings/course-information"
                class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/course-information")
                ? "button--blue"
                : "button--transparent"}"
              >
                <i class="bi bi-journal-text"></i>
              ...
              <a
                ...
                class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/tags")
                ? "button--blue"
                : "button--transparent"}"
              >
                <i
              ...
              <a
                ...
                class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/invitations")
                ? "button--blue"
                : "button--transparent"}"
              >
                <i
              ...
              <a
                ...
                class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/enrollments")
                ? "button--blue"
                : "button--transparent"}"
              >
                <i
              ...
              <a
                ...
                class="dropdown--menu--item menu-box--item button ${req.path.endsWith("/settings/your-enrollment")
                ? "button--blue"
                : "button--transparent"}"
              >
                <i
    ...
    app.get("/courses/:courseReference/settings/course-information", ...app.locals.middlewares.isCourseStaff, (req, res) => {
        res.send(courseSettingsLayout({
            req,
            res,
            head: html `
            <title>
                ...
                : html `
                    ...
                    >
                      <span>
                        Archived
                        <time
                          datetime="${new Date(res.locals.course.archivedAt).toISOString()}"
                          onload="${javascript `
    ...
    app.patch("/courses/:courseReference/settings/course-information", (req, res, next) => {
        res.locals.actionAllowedOnArchivedCourse =
            typeof req.body.isArchived === "string";
        next();
    }, ...app.locals.middlewares.isCourseStaff, (req, res, next) => {
        ...
        if (typeof req.body.isArchived !== "string") {
            ...
            SET "name" = ${req.body.name},
                "year" = ${typeof req.body.year === "string" &&
                req.body.year.trim() !== ""
                ? req.body.year
                : null},
                "term" = ${typeof req.body.term === "string" &&
                req.body.term.trim() !== ""
                ? req.body.term
                : null},
                "institution" = ${typeof req.body.institution === "string" &&
                req.body.institution.trim() !== ""
                ? req.body.institution
                : null},
          ...
          `);
            ...
            app.locals.database.run(sql `
            UPDATE "courses"
            SET "archivedAt" = ${req.body.isArchived === "true" ? new Date().toISOString() : null}
            WHERE "id" = ${res.locals.course.id}
          `);
            app.locals.helpers.Flash.set({
                req,
        ...
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/settings/course-information`);
        app.locals.helpers.liveUpdatesDispatch({ req, res });
    });
    app.get("/courses/:courseReference/settings/tags", ...app.locals.middlewares.isCourseStaff, (req, res) => {
        res.send(courseSettingsLayout({
            req,
            res,
            head: html `
            ...
            >
              ...
              >
                <div
                  ...
                  `)}"
                >
                  $${res.locals.tags.map((tag, index) => html `
                      <div
                ...
                >
                  <button
                    ...
                    onload="${javascript `
                      const newTagPartial = ${res.locals.html(html `
                          <div
                            class="tag"
                            css="${res.locals.css(css `
                              padding-bottom: var(--space--4);
    ...
    app.put("/courses/:courseReference/settings/tags", ...app.locals.middlewares.isCourseStaff, (req, res, next) => {
        if (!Array.isArray(req.body.tags) ||
            req.body.tags.length === 0 ||
            req.body.tags.some((tag) => (tag.reference === undefined &&
                (typeof tag.name !== "string" ||
                    tag.name.trim() === "" ||
                    ![undefined, "on"].includes(tag.isStaffOnly))) ||
                (tag.reference !== undefined &&
                    (!res.locals.tags.some((existingTag) => tag.reference === existingTag.reference) ||
                        (tag.delete !== "true" &&
                            (typeof tag.name !== "string" ||
                                tag.name.trim() === "" ||
                                ![undefined, "on"].includes(tag.isStaffOnly)))))))
        ...
        for (const tag of req.body.tags)
            ...
            `);
              ...
              UPDATE "tags"
              SET "name" = ${tag.name},
                  "staffOnlyAt" = ${tag.isStaffOnly === "on" ? new Date().toISOString() : null}
        ...
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/settings/tags`);
        app.locals.helpers.liveUpdatesDispatch({ req, res });
    });
    app.get("/courses/:courseReference/settings/invitations", ...app.locals.middlewares.isCourseStaff, (req, res) => {
        const invitations = app.locals.database.all(sql `
            ...
            >
              ...
              <div class="label">
                ...
                <div
                  ...
                  `)}"
                >
                  $${courseRoles.map((courseRole) => html `
                        <label
                          class="button button--tight button--tight--inline button--transparent"
                        >
                          <input
            ...
            $${invitations.length === 0
                ...
                : html `
                  $${invitations.map((invitation) => {
                    ...
                    const isInvitationExpired = app.locals.helpers.isExpired(invitation.expiresAt);
                      ...
                      >
                        ...
                        ? html `
                            ...
                            return res.locals.html(html `
                                ...
                                : html ``}
                                    ...
                                    `}"
                                  >
                                    ${"*".repeat(6)}${invitation.reference.slice(6)}
                        ...
                        : html `
                          ...
                          >
                            ...
                            >
                              <button
                                class="button button--tight button--tight--inline button--transparent ${courseRoleTextColor[invitation.courseRole]}"
                                onload="${javascript `
                                  (this.tooltip ??= tippy(this)).setProps({
                                    touch: false,
                        ...
                        ? "button--blue"
                        : "button--transparent"} ${courseRoleTextColor[courseRole]}"
                                                    $${isUsed
                        ? html `
                                                          type="button"
                                                          onload="${javascript `
                        ...
                        : isInvitationExpired
                            ...
                            : html ``}
                                                  >
                                                    $${courseRoleIcon[courseRole][courseRole === "staff"
                        ? "fill"
                        : "regular"]}
                                                    ${lodash.capitalize(courseRole)}
                                                  </button>
                                ...
                                `}"
                              >
                                $${courseRoleIcon[invitation.courseRole][invitation.courseRole === "staff"
                        ? "fill"
                        : "regular"]}
                                  ...
                                  >
                                    ...
                                    <div class="dropdown--menu--item">
                                      <input
                                        type="text"
                                        name="expiresAt"
                                        value="${new Date(invitation.expiresAt ?? new Date()).toISOString()}"
                                        required
                                        autocomplete="off"
                                        class="input--text"
                                        onload="${javascript `
                                            ...
                                            (this.tooltip ??= tippy(this)).setProps({
                                              ...
                                              content: ${res.locals.html(html `
                                                  <div>
                                                    Used
                                                    <time
                                                      datetime="${new Date(invitation.usedAt).toISOString()}"
                                                      onload="${javascript `
                                                      leafac.relativizeDateTimeElement(this, { preposition: "on", target: this.parentElement });
                                                    `}"
                                                    ></time>
    ...
    const invitationMailer = ({ req, res, invitation, }) => {
    ...
    app.post("/courses/:courseReference/settings/invitations", ...app.locals.middlewares.isCourseStaff, (req, res, next) => {
        ...
        switch (req.body.type) {
            case "link":
                const invitation = app.locals.database.get(sql `
          ...
          `);
            ...
            case "email":
                ...
                if (emails.length === 0 ||
                    emails.some(({ email }) => email.match(app.locals.helpers.emailRegExp) === null))
                    return next("validation");
                for (const { email, name } of emails) {
                    if (app.locals.database.get(sql `
                  SELECT TRUE
                  FROM "enrollments"
                ...
                `) !== undefined)
                        continue;
                    const existingUnusedInvitation = app.locals.database.get(sql `
                SELECT "id", "name"
                FROM "invitations"
        ...
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/settings/invitations`);
    });
    const invitationExistsMiddleware = [
        (req, res, next) => {
            const invitation = app.locals.database.get(sql `
    ...
    app.patch("/courses/:courseReference/settings/invitations/:invitationReference", ...app.locals.middlewares.isCourseStaff, ...invitationExistsMiddleware, (req, res, next) => {
        ...
        if (req.body.courseRole !== undefined) {
            if (app.locals.helpers.isExpired(res.locals.invitation.expiresAt) ||
                !courseRoles.includes(req.body.courseRole))
                return next("validation");
            app.locals.database.run(sql `UPDATE "invitations" SET "courseRole" = ${req.body.courseRole} WHERE "id" = ${res.locals.invitation.id}`);
            app.locals.helpers.Flash.set({
                req,
                res,
                theme: "green",
        ...
        if (req.body.expiresAt !== undefined) {
            if (typeof req.body.expiresAt !== "string" ||
                ...
                app.locals.helpers.isExpired(req.body.expiresAt))
                return next("validation");
            app.locals.database.run(sql `UPDATE "invitations" SET "expiresAt" = ${req.body.expiresAt} WHERE "id" = ${res.locals.invitation.id}`);
            app.locals.helpers.Flash.set({
                req,
                res,
                theme: "green",
    ...
    app.get("/courses/:courseReference/settings/enrollments", ...app.locals.middlewares.isCourseStaff, (req, res) => {
        ...
        res.send(courseSettingsLayout({
          ...
          `,
            ...
            $${enrollments.map((enrollment) => {
                ...
                const isOnlyStaff = isSelf &&
                    enrollments.filter((enrollment) => enrollment.courseRole === "staff").length === 1;
                return html `
                <div
                  key="enrollment--${enrollment.reference}"
                  class="enrollment"
                ...
                >
                  ...
                  >
                    <div>
                      <div
                        data-filterable-phrases="${JSON.stringify(app.locals.helpers.splitFilterablePhrases(enrollment.user.name))}"
                        class="strong"
                      >
                      ...
                      <div class="secondary">
                        <span
                          data-filterable-phrases="${JSON.stringify(app.locals.helpers.splitFilterablePhrases(enrollment.user.email))}"
                          css="${res.locals.css(css `
                        ...
                        <button
                          ...
                          onload="${javascript `
                            ...
                            this.onclick = async () => {
                              await navigator.clipboard.writeText(${JSON.stringify(enrollment.user.email)});
                              this.copied.show();
                      ...
                      >
                        <span>
                          Last seen online
                          <time
                            datetime="${new Date(enrollment.user.lastSeenOnlineAt).toISOString()}"
                            onload="${javascript `
                              leafac.relativizeDateTimeElement(this, { preposition: "on", target: this.parentElement });
                            `}"
                          ></time>
                    ...
                    >
                      ...
                      >
                        <button
                          class="button button--tight button--tight--inline button--transparent ${courseRoleTextColor[enrollment.courseRole]}"
                          onload="${javascript `
                            (this.tooltip ??= tippy(this)).setProps({
                              touch: false,
                              content: "Update Course Role",
                    ...
                    : "regular"]}
                          ...
                          `}"
                        >
                          $${courseRoleIcon[enrollment.courseRole][enrollment.courseRole === "staff"
                    ? "fill"
                    : "regular"]}
    ...
    const mayManageEnrollmentMiddleware = [
        ...app.locals.middlewares.isCourseStaff,
        (req, res, next) => {
            const managedEnrollment = app.locals.database.get(sql `
          SELECT "id", "reference"
          FROM "enrollments"
          WHERE "course" = ${res.locals.course.id} AND
                "reference" = ${req.params.enrollmentReference}
        `);
            if (managedEnrollment === undefined)
                return next("route");
            res.locals.managedEnrollment = {
                ...managedEnrollment,
                isSelf: managedEnrollment.id === res.locals.enrollment.id,
            };
            if (res.locals.managedEnrollment.isSelf &&
    ...
    app.patch("/courses/:courseReference/settings/enrollments/:enrollmentReference", ...mayManageEnrollmentMiddleware, (req, res, next) => {
        if (typeof req.body.courseRole === "string") {
            if (!courseRoles.includes(req.body.courseRole))
                return next("validation");
            app.locals.database.run(sql `UPDATE "enrollments" SET "courseRole" = ${req.body.courseRole} WHERE "id" = ${res.locals.managedEnrollment.id}`);
            app.locals.helpers.Flash.set({
                req,
                res,
                theme: "green",
        ...
        res.redirect(303, res.locals.managedEnrollment.isSelf
    ...
    app.delete("/courses/:courseReference/settings/enrollments/:enrollmentReference", ...mayManageEnrollmentMiddleware, (req, res) => {
        app.locals.database.run(sql `DELETE FROM "enrollments" WHERE "id" = ${res.locals.managedEnrollment.id}`);
        app.locals.helpers.Flash.set({
            req,
            res,
            theme: "green",
    ...
    app.get("/courses/:courseReference/settings/your-enrollment", ...app.locals.middlewares.isEnrolledInCourse, (req, res) => {
        res.send(courseSettingsLayout({
            req,
            res,
            head: html `
            ...
            >
              ...
              <div class="label">
                ...
                <div
                  ...
                  `)}"
                >
                  $${enrollmentAccentColors.map((accentColor) => html `
                      <input
                        type="radio"
                        name="accentColor"
                        value="${accentColor}"
    ...
    app.patch("/courses/:courseReference/settings/your-enrollment", ...app.locals.middlewares.isEnrolledInCourse, (req, res, next) => {
        if (typeof req.body.accentColor !== "string" ||
            !enrollmentAccentColors.includes(req.body.accentColor))
            return next("validation");
        app.locals.database.run(sql `UPDATE "enrollments" SET "accentColor" = ${req.body.accentColor} WHERE "id" = ${res.locals.enrollment.id}`);
        app.locals.helpers.Flash.set({
            req,
            res,
            theme: "green",
        ...
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}/settings/your-enrollment`);
    });
    const isInvitationUsableMiddleware = [
    ...
    app.get("/courses/:courseReference/invitations/:invitationReference", ...app.locals.middlewares.isEnrolledInCourse, ...isInvitationUsableMiddleware, asyncHandler(async (req, res) => {
        if (typeof req.query.redirect === "string" &&
            req.query.redirect.trim() !== "" &&
            req.query.redirect.startsWith("/"))
            res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.course.reference}${req.query.redirect}`);
        const link = `https://${app.locals.options.host}/courses/${res.locals.course.reference}/invitations/${res.locals.invitation.reference}`;
        res.send(app.locals.layouts.box({
            req,
            res,
            head: html `
          ...
          `,
            ...
            <div>
              ...
              $${(await QRCode.toString(`https://${app.locals.options.host}/courses/${res.locals.course.reference}/invitations/${res.locals.invitation.reference}`, { type: "svg" }))
    ...
    app.get("/courses/:courseReference/invitations/:invitationReference", ...app.locals.middlewares.isSignedIn, ...isInvitationUsableMiddleware, (req, res) => {
        res.send(app.locals.layouts.box({
            req,
            res,
            head: html `
    ...
    app.post("/courses/:courseReference/invitations/:invitationReference", ...app.locals.middlewares.isSignedIn, ...isInvitationUsableMiddleware, (req, res) => {
        app.locals.database.run(sql `
          INSERT INTO "enrollments" ("createdAt", "user", "course", "reference", "courseRole", "accentColor")
          VALUES (
            ${new Date().toISOString()},
        ...
        if (res.locals.invitation.email !== null)
            ...
            WHERE "id" = ${res.locals.invitation.id}
          `);
        res.redirect(303, `https://${app.locals.options.host}/courses/${res.locals.invitation.course.reference}${typeof req.query.redirect === "string" &&
            req.query.redirect.trim() !== "" &&
            req.query.redirect.startsWith("/")
            ? req.query.redirect
            : "/"}`);
    });
    app.get("/courses/:courseReference/invitations/:invitationReference", ...app.locals.middlewares.isSignedOut, ...isInvitationUsableMiddleware, (req, res) => {
        res.send(app.locals.layouts.box({
            req,
            res,
            head: html `
          ...
          `,
            ...
            >
              $${(() => {
                ...
                const invitationUserExists = hasInvitationEmail &&
                    app.locals.database.get(sql `
                      SELECT EXISTS(
                        SELECT TRUE
                        FROM "users"
    ...
    app.get("/courses/:courseReference/invitations/:invitationReference", (req, res) => {
        res.send(app.locals.layouts.box({
            req,
            res,
