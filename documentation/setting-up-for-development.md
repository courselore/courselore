...
> **Note:** Courselore uses the following network ports: 80, 443, and 6000–9000. Stop other applications you may have running on those ports. In macOS and Linux, for example, you may find which application is running on a network port by using `lsof -i:80`. Or you may prefer to use [`npx kill-port 80 443 ...`](https://github.com/tiaanduplessis/kill-port) if you already have Node.js installed.
...
When you’re working on a page that may be slow or on infrastructure, you should measure the performance.

We use two tools to do this:

1. [0x](https://npm.im/0x): Profile the application and generate [flame graphs](https://www.brendangregg.com/flamegraphs.html).
2. [autocannon](https://npm.im/autocannon): Load test (send lots of requests) and measure response time (which autocannon calls “latency”) & throughput.
