(function (global, factory) {
	...
	[Symbol.iterator](){return this;};next(){const{state,result}=this;if(this.c>1){result.done=true;result.value=undefined;return result;}
	...
	exports.diff_core = bin.diff_core=diff_core;function diff(xs,ys){let[i,N,M]=[0,xs.length,ys.length];while(i<N&&i<M&&xs[i]===ys[i])
	i++;if(i===N&&i===M)
	return [][Symbol.iterator]();while(xs[--N]===ys[--M]&&N>i&&M>i);const eq=(x,y)=>xs[x]===ys[y];return diff_core(i,N+1-i,i,M+1-i,eq);}
	exports.diff = bin.diff=diff;class LCSGen{constructor(diff,N){this.diff=diff;this.N=N;this.i=0;this.j=0;}
	[Symbol.iterator](){return this;};next(){const rec=this.diff.next();if(rec.done){const{i,j,N}=this;if(i<N){rec.done=false;rec.value=[i,j,N-i];this.i=N;}
	return rec;}
	...
	function lcs(xs,ys){return new LCSGen(diff(xs,ys),xs.length);}
	exports.lcs = bin.lcs=lcs;function*calcPatch(xs,ys){const slice=ArrayBuffer.isView(xs)?Uint8Array.prototype.subarray:xs.slice;for(const v of diff(xs,ys)){v[2]=slice.call(ys,v[2],v[3]);yield v;}}
	exports.calcPatch = bin.calcPatch=calcPatch;function*applyPatch(xs,patch){let i=0;const slice=ArrayBuffer.isView(xs)?Uint8Array.prototype.subarray:xs.slice;for(const[dels,dele,ins]of patch){if(i<dels)
	yield slice.call(xs,i,dels);if(ins.length>0)
	yield ins;i=dele;}
	if(i<xs.length)
	yield slice.call(xs,i);}
	exports.applyPatch = bin.applyPatch=applyPatch;
